...
SELECT EMP_ID, EMP_NAME, EMP_NAME FROM EMPLOYEE;
...
/*
 * 컬럼명 AS 별칭 : 별칭쓸때 띄어쓰기 쓰면 좆됌 특수문자도 안됌 문자만 가능
...
-- 주의사항 2) DISTINECT 구문은 SELECT 제일 앞에 작성되어야 한다
...
-- EMPLOYEE 테이블에서 급여가 300만 미만 또는 500만 이상의 사원의
-- 사번, 이름, 급여, 전화번호 조회
...
--NOT BETWEEN A AND B : A이상 B 이하가 아닌 경우 --> A 미만, B초과
...
SELECT EMP_NAME, HIRE_DATE FROM EMPLOYEE WHERE HIRE_DATE BETWEEN '1990-01-01' AND '1999-12-13';
...
-- '_' 예시
-- 'A_' : A로 시작하는 두 글자 문자열 A로 시작하고 뒤에 아무문자나 와도됨
-- '___A' : A로 끝나는 네 글자 문자열 
-- '__A__' : 3번째 글자가 A인 다섯 글자 문자열 
-- '_____' : 어떤 문자던 간에 5글자 문자열 
...
--EMAIL 에서 _ 앞에 글자가 세 글자인 사원 이름 , 이메일 조회

SELECT EMP_NAME, EMAIL FROM EMPLOYEE WHERE EMAIL LIKE '___$_%' ESCAPE '$';

-- 문제점 : _ 를 기준점으로 삼았으나 , 패턴 중 '_' 와 동일한 표기법으로 작성되어 구분되지않음

-- 해결 방법 : LIKE의 ESCAPE OPTION 을 이용하여 _ 를 구분한다

-- LIKE EXCAPE OPTION : 일반 문자로 처리할 '_' / % 앞에 아무 특수기호나 첨부해서 구분하게함

--> $ 뒤에 작성된 _는 일반문자로 탈출시킨다
...
SELECT EMP_ID, EMP_NAME, EMAIL, DEPT_CODE ,HIRE_DATE, SALARY FROM EMPLOYEE WHERE EMAIL LIKE '____$_%' ESCAPE '$' AND (DEPT_CODE = 'D9' OR DEPT_CODE = 'D6') AND 
HIRE_DATE BETWEEN '1990-01-01' AND '2000-12-31' AND SALARY >= 2700000;
...
---부서코드가 D1, D6, D9가 아닌 사원의 사번,이름 부서코드 조회

SELECT EMP_ID, EMP_NAME, DEPT_CODE 
FROM EMPLOYEE 
WHERE DEPT_CODE NOT IN('D6', 'D1', 'D9')
 ...
 * 2.WHERE절
 * 4.그 다음에 ORDER BY 절 
 * ORDER BY 컬럼명 | 별칭 | 컬럼순서 [ASC(오름차순) | DESC(내림차순)] [NULLS FIRST
 * | LAST](오름차순 : 기본값)(내림차순) 대괄호는 안써도됨 
 * 
 * */
