 ...
 * 3. GROUP BY 그룹을 묶을 컬러명 작성
 * 4. HAVING : 그룹함수식 비교연산자 비교값 = WHERE 절 하고 똑같다
 * 6. 정렬이 필요하면 마지막으로 ORDER BY 컬럼명, 숫자 [NULLS FIRST | LAST]
 * 
 * 
 * */
...
-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 그룹이 여러개일 경우 오류가 발생
-- 여러 개의 결과값을 산출하기 위해 그룹함수가 적용된 그룹의 기0준을 ORDER BY 절에 기술하여 사용
...
--EMPLOYEE 테이블에서 직급별 2000년도 이후(2000년 포함) 입사자들의
-- 급여 합을 조회 (직급코드 오름차순)

SELECT JOB_CODE, SUM(SALARY) "급여 평균"
FROM EMPLOYEE
...
-- EMPLOYEE 에서 부서코드가 'D5' 인 사원의 사번, 이름, 부서코드 , 급여
