 ...
 * 서브쿼리 유형
 * 
 * - 단일행 (단일열)서브쿼리 : 서브쿼리의 조회 결과 값이 개수가 1개일 때
 * 
...
SELECT EMP_ID , EMP_NAME , DEPT_TITLE, DEPT_ID, SALARY , HIRE_DATE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
WHERE SALARY = ( SELECT MIN(SALARY) FROM EMPLOYEE );

-- 노옹철 ㅅ원의 급여보다 많이 받는 직원의
-- 사번, 이름 부서명, 직급명 , 급여조회
...
/*
 *  >> 다중행 서브쿼리 앞에는 일반 비교연산자는 사용 X
 *  
 *  - IN / NOT IN : 여러 개의 결과값 중에서 한개라도 일치하는 값이 있따면 혹은 없다면 이라는 의미 (가장많이 사용)
 * 
...
-- 서브쿼리에서 IN 은 여기 안에서 (서브쿼리 조건 ) 


-- 사수에 해당하는 직원에 대해 조회
-- 사번, 이름, 부서명, 직급명, 구분(사수/직원)

-- 사수 == MANEGER_ID 컬럼에 작성된 사번
...
--3 대리 직급의 직원들 중에서 과장 직급의 최소 급여보다 많이 받는 직원 조회
...
-- DEPARYMENT 테이블의 LOCATION_ID와 동일한 DEPT_ID 가
-- EMPLOYEE 테이블의 DEPT_CODE의 동일한 사원을 구하시오

SELECT LOCAL_CODE FROM LOCATION WHERE NATIONAL_CODE = 'KO';


--2) DEPARTMENT 테이블에서 위의 결과와 동일한 LOCATION_ID 를 가지고 있는
-- DEPT_ID를 조회해야한다
...
--3)최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한 DEPT_CODE를 가진 사람을 조회
...
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE EXTRACT(YEAR FROM HIRE_DATE) LIKE '2000%'; --유재식
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D9','D6') AND SALARY >= 3000000 AND BONUS IS NOT NULL AND 
 ...
 * --> 선동일은 JOB_CODE가 'J1'이고, SALARY 가 8백만
 * 
 ...
 *  
 * 결론 : 선동일은 조건을 만족하지않으므로 메인쿼리가
 *        선동일은 RESULT SET 에서 제외함  
 * 
 * 다음 . . ------------------------------------------------------
 * 
 * 1. 메인쿼리에서 다음 1행을 읽음(2행 - 송중기)
 * --> 송중기는 'J2'이고 , 6백만 받음
 * 
 ...
 * 
 * 3. 메인쿼리가 서브쿼리의 수행결과를 이용하여
 * 		송중기가 평균 급여보다 초과하여 받는지 판별함
 *    --> J2이고 , 평균 급여보다 초과하여 받음
 * 
 * 
...
WHERE MAIN.MANAGER_ID = SUB.EMP_ID) ;
  ...
  * 따라서 각 직원의 MANAGER_ID 가 다른 직원의 EMP_ID와 일치하는 경우에
  * 해당 직원의 정보를 반환함
  * 
  * 
  * 
...
/*
 * 
 * EXISTS 서브쿼리는 조건을 만족하는 데이터가 하나 이상 존재하는지 확인하고
 * 실제 데이터 값으 반환하지 않음
 * 즉, 각 직원이 MANAGER_ID 가 다른 직원의 EMP_ID 와 일치하는 경우만 확인됨
 * 
 * --> 서브쿼리 결과가 메인쿼리의 행들 중 일치하는게 있냐? 
...
WHERE DEPT_CODE = 'D1';
/*
 * 메인쿼리의 첫번째 행인 방명수가 있는 부서 D1의 가장빠른 입사일을 조회했는데
 * 2007 - 03 - 20 
 *  방명수 입사일이랑 다름  방명수는 아님 RESULT SET 에서 제외
 * 
 * 2. 차태연 입사일이랑 다름. 차태연 아님 (RESULT SET에서 제외)
...
SELECT EMP_ID, EMP_NAME, MANAGER_ID,
   					NVL((SELECT EMP_NAME 
   					FROM EMPLOYEE SUB
   					WHERE SUB.EMP_ID = MAIN.MANAGER_ID                  ), '없음') AS관리자명
FROM EMPLOYEE MAIN;
...
-- RANK() OVER : 동일한 순위 이후의 등수를 동일한 인원 수 만큼 건너뛰고
-- 순위 계산
...
--RANK() OVER(정렬순서)
SELECT RANK() OVER(ORDER BY SALARY DESC) || '위'0 , EMP_NAME , SALARY FROM EMPLOYEE
; 

-- DENSE_RANK() OVER() : 동일한 순위 이후의 등수를 이후의 순위로 계산 
--EX) 공동 1위가 2명이어도 다음 순위가 2위
SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC) , EMP_NAME ,SALARY FROM EMPLOYEE;
