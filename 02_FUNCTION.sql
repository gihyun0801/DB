...
-- 함수는 SELECT문의
-- SELECT 절, WHERE 절, ORDER BY 절, GROUP BY 절, HAVING 절 사용가능
...
SELECT TRIM(BOTH '#' FROM '####안녕####') -- 특정 문자 제거
FROM DUAL;
...
-- MOD(숫자|컬러명, 숫자|컬럼명) : 나머지 값 반환
--EMPLOYEE 테이블에서 사원의 월급을 100만으로 나눴을때 마지막 조회

SELECT EMP_NAME, SALARY, MOD(SALARY ,1000000)
FROM EMPLOYEE;

--EMPLOAYEE 테이블에서 사번이 짝수인 사원의 사번, 이름 조회

--EMPLOAYEE 테이블에서 사번이 홀수인 사원의 사번, 이름 조회
...
SELECT ROUND(123.456) -- 아무것도 안적으면 소숫점 첫번째 자리에서 반올림
FROM DUAL;

SELECT ROUND(123.456, 1) -- 1적으면 소숫점 2번째 자리에서 반올림
FROM DUAL;

SELECT ROUND(123.456, 2) -- 1적으면 소숫점 2번째 자리에서 반올림
FROM DUAL;

SELECT ROUND(123.456, -1) FROM DUAL;-- 소수점 0번째 자리에서 반올림해서 
  												-- 소수점 -1 자리 표현
                           -- == 1의 자리에서 반올림해서 10자리부터 표현
...
--TRUNC(숫자 | 컬럼명[, 위치])  : 특정 위치서부터 버림 (절삭)
...
CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)   / 12 ) ||'년차' "근무한 년수"
FROM EMPLOYEE;
...
-- LAST DAY(날짜) : 해당 달의 마지막 날짜를 구함

SELECT LAST_DAY(SYSDATE) FROM DUAL;
SELECT LAST_DAY('2024-03-01') FROM DUAL;

SELECT EMP_NAME, EXTRACT(YEAR FROM HIRE_DATE) || EXTRACT(MONTH FROM HIRE_DATE) 
FROM EMPLOYEE;
...
SELECT TO_DATE('2024-02-29') FROM DUAL;
SELECT TO_DATE(20240229) FROM DUAL;

SELECT TO_DATE('240229 123350', 'YYMMDD HH24 MI SS') FROM DUAL;
...
SELECT TO_DATE(SUBSTR(EMP_NO)
FROM EMPLOYEE;

SELECT TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') - 1), 'RRMMDD'), 'YYYY"년"MM"월"DD"일"') AS 생년월일
FROM EMPLOYEE;

 
-- Y패턴 : 현재 세기 (21세기 == 20XX년대 == 2000년대)
-- R패턴 : 1세기를 기준으로 절반(50년) 이상인 경우 이전세기(1900)
 					-- 절반(50년) 미만인 경우 현재 세기(2000년대)

SELECT TO_DATE('510505', 'YYMMDD') FROM DUAL;
SELECT TO_DATE('510505', 'RRMMDD') FROM DUAL;
SELECT TO_DATE('410505', 'RRMMDD') FROM DUAL;
...
SELECT TO_NUMBER('1,000,100', '9,999,999') + 5000000 FROM DUAL;
...
--NULL 과 산술연산을 진행하면 무조건 결과는 NULL 이나온다
...
--EMPLOYEE 에서 보너스를 받는직원이면 'O' 안받으면 'X'
...
SELECT EMP_NAME,JOB_CODE, SALARY , DECODE( JOB_CODE, 'J7', '20%'
 																											'J6', '15%'
 																											 'J5', '10%',
 																											  '5%') 인상률,
 																  DECODE(JOB_CODE, 'J7', SALARY * 1.2,
 																							'J6', SALARY * 1.15,
 																											                     'J5', SALARY, 1.1,
 																											                      SALARY * 1.05) "인상된 급여"  FROM EMPLOYEE;
...
-- 급여 최대값, 가장늦은 입사일, 알파벳 순서가 가장느린 입사일
 																											                     
 	SELECT MAX(SALARY), MAX(HIRE_DATE), MAX(EMAIL)
FROM EMPLOYEE;											

--EMPLOYEE 에서 급여를 가장 많이 받는 사원의
-- 이름 , 급여 , 직급코드를 조회
...
--> 왜 1행이 적을까 ? NU;; 값이 포함되지 않아서
-- COUNT(컬럼명)에 의해 NULL을 제외한 실제 값이 있는 행의 개수만 조회


--EMPLOYEE 성별이 남성인 사원의 수 조회
